...
def spreadsheet_reader(spreadsheet):
    ...
    for r in range(2,ws.max_row+1):
        colunas = [{'campo'+str(c):ws.cell(row=r,column=c).value} for c in range(1,ws.max_column+1) if ws.cell(row=r,column=1).value!='Dia Mes']
        dados_importados.append(colunas)
    return dados_importados

def spreadsheet_reader_backup(spreadsheet):
    data = []
    dicionario = []
    wb = load_workbook(filename=spreadsheet)
    ws = wb.active
