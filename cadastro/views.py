...
def gerador(request):
    ...
    # mes_json = open(os.path.join(BASE_DIR,'planilha.json'),'w')
    ...
    if 'ano' in request.POST:
        ...
        planilha_dict={'ano':ano,'mes':mes,'nome mes':mes_label}
        # json.dump({'ano':ano},mes_json,indent=4)
        # json.dump({'mes':mes,'nome mes':mes_label},mes_json,indent=4)
        for dia_mes in range(1,ultimo_dia+1):
            ...
            if dia_semana==0:
                ...
                planilha_dict[dia_mes]=lista_segunda
            elif dia_semana==1:
                ...
                swap_xls(lista_dia,wsMes,dia_mes)
                grava_json(BASE_DIR,lista_mes)
                planilha_dict[dia_mes]=lista_terca
            elif dia_semana==2:
                ...
                swap_xls(lista_dia,wsMes,dia_mes)
                grava_json(BASE_DIR,lista_mes)
                planilha_dict[dia_mes]=lista_quarta
            elif dia_semana==3:
                ...
                swap_xls(lista_dia,wsMes,dia_mes)
                grava_json(BASE_DIR,lista_mes)
                planilha_dict[dia_mes]=lista_quinta
            elif dia_semana==4:
                ...
                swap_xls(lista_dia,wsMes,dia_mes)
                grava_json(BASE_DIR,lista_mes)
                planilha_dict[dia_mes]=lista_sexta
            elif dia_semana==5:
                ds='Sabado';print(ds)
                dia=filling_header()
                dia['dia_mes']=dia_mes
                ...
                swap_xls(lista_dia,wsMes,dia_mes)
                grava_json(BASE_DIR,lista_mes)
                planilha_dict[dia_mes]=lista_sabado
    with open(os.path.join(BASE_DIR,'planilha.json'),'w') as planilha_file:
        json.dump(planilha_dict,planilha_file,indent=4)
    with open(os.path.join(BASE_DIR,'planilha.json')) as planilha_file:
        planilha_retorno = json.load(planilha_file)
        for linha in planilha_retorno:
            print('linha da planilha gerada {}'.format(linha))
...
def grava_json(BASE_DIR,lista_mes):
    with open(os.path.join(BASE_DIR,'lista_mes.json'),'w') as f:
...
def importar(request):
    ...
    if request.method=='POST':
        ...
        context = leitor
        with open(os.path.join(BASE_DIR,'importacao.json'),'w') as imp:
            json.dump(context,imp,indent=4)
...
def gravar(request):
    ...
    #problemas para conseguir pegar o conteudo de um post, entao esta sendo tratado o json que foi gerado em paralelo ao post
    if request.method =='POST':
        with open(os.path.join(BASE_DIR,'importacao.json')) as file_import:
            for linha in json.load(file_import):
                for periodos_lista in linha:
                    print('lista {}'.format(periodos_lista))
                
                    # print('periodo: {} {}'.format(linha,type(linha)))
                    # for k,v in dict.items():
                        # print('chave {} {}'.format(k,v))
                        # if k=='Dia Mes'
                # print('json importado {}'.format(linha))
