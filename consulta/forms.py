...
class designacaoForms(forms.Form):
    ...
    sel_irmaos = forms.ModelChoiceField(queryset=irmaos.objects.all().filter(habilitado='True').order_by('nm'),required=False,label='selecione o Irmao',widget=forms.Select(attrs={'readonly':'readonly'}))
